...
import { ArcElement, Chart as ChartJS, Tooltip, type TooltipItem } from "chart.js";
...
export function TransportBreakdownChart({ breakdown }: { breakdown: TransportBreakdown[] }) {
    ...
    <div className="flex flex-col items-center gap-6 md:flex-row md:items-start">
      <div className="relative h-48 w-48 shrink-0">
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-2xl font-semibold text-foreground">{totalFormatted.value}</span>
          <span className="text-xs text-muted-foreground">{totalFormatted.unit} CO2e</span>
        </div>
      </div>
      ...
      <ul className="w-full flex-1 space-y-3">
        {breakdown.map((entry) => {
          ...
          return (
            <li key={entry.transportType} className="flex items-center gap-3 text-sm">
              <span
                aria-hidden="true"
                className="h-2.5 w-2.5 shrink-0 rounded-full"
                style={{ backgroundColor: `var(${TRANSPORT_COLORS[entry.transportType]})` }}
              />
